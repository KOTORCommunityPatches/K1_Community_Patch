...
/*	KOTOR Community Patch
	...
	prisoner in the Korriban Sith Academy when the player successfully gets the
	location of the Mando's cache (before they overdose and die). Like the other
	exit scripts, it has been tweaked to reposition the Mando before killing him
	in an attempt to prevent him clipping through the cage as much as possible
	when playing the dead animation. Also sets the corpse to non-destroyable so
	that it sticks around if the player chooses to kill the torturer.
	
	See also k35_init_torture, k35_plc_startvic, k35_tor_move, k35_vic_collapse,
	k35_vic_drugged, k_pkor_destvictm, k_pkor_painreset, k_pkor_tort01.
