...
/*	KOTOR Community Patch
	...
	UserDefine for kas23_chuunda_01.utc in kas_m23ad (Kashyyyk Chieftain's Hall).
	
	This is Chuundar's UserDefine script. When siding with him, his OnHeartbeat
	event will kick in as soon as Freyyr and his allies are all dead, starting
	...
	to their pre-fight starting positions and everyone is healed. This is more or
	less identical to Freyyr's UserDefine, aside from the OnDeath function.
	
	See also k_pkas_freyyuse2.
			...
			if (GetFinalBattleLocal() == TRUE && GetIsObjectValid(GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF)) == FALSE && GetIsConversationActive() == FALSE)
				{
					...
					location lChun = Location(Vector(13.98,6.97,3.79), 56.25);
					...
					// Jump the party, Zaalbar, and Chuundar under the fadeout before starting the conversation.
					CP_PartyJumpObject(oPC, oPCWP);
					CP_PartyJumpObject(oPM1, oPM1WP);
					CP_PartyJumpObject(oPM2, oPM2WP);
					CP_PartyJump(OBJECT_SELF, lChun);
					CP_PartyJump(oZaal, lZaal);
					
					DelayCommand(0.9, AssignCommand(oPC, SetFacingPoint(GetPositionFromLocation(lChun))));
					DelayCommand(1.0, AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_LISTEN, 1.0, 0.5)));
					DelayCommand(0.9, AssignCommand(oPM1, SetFacingPoint(GetPositionFromLocation(lChun))));
					DelayCommand(0.9, AssignCommand(oPM2, SetFacingPoint(GetPositionFromLocation(lChun))));
